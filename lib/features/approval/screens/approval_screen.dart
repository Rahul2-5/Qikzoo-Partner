    ...
    return Scaffold(
      ...
      body: approvalAsync.when(
        ...
        data: (status) => Center(child: Padding(
          ...
          child: _ApprovalCard(status: status),
        )),
      ),
    );
  }
...
class _ApprovalCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (title, message, icon, color) = switch (status.state) {
      ApprovalState.pending => ('Application pending', 'We have received your application.', Icons.schedule_outlined, AppColors.warning),
      ApprovalState.underReview => ('Application under review', 'Our team is reviewing your submitted documents.', Icons.manage_search_outlined, AppColors.primary),
      ApprovalState.approved => ('You are approved', 'You can now go online and start delivering.', Icons.verified_outlined, AppColors.secondary),
      ApprovalState.rejected => ('Application needs attention', status.rejectionReason ?? 'Please update your application and try again.', Icons.error_outline, AppColors.error),
    };
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(AppSpacing.lg),
      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(AppRadius.sheet), boxShadow: AppShadows.card),
