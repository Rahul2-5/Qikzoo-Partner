class Validators {
  ...
  /// Validates an Indian mobile number before an OTP can be requested.
  ///
  /// In addition to the Indian mobile-number format, reject values such as
  /// `9999999999`. They are commonly used as placeholder values and should
  /// never consume an OTP request.
  static bool isValidPhone(String value) {
    final phone = value.trim();
    return _phoneRegex.hasMatch(phone) && !_hasRepeatedDigits(phone);
  }

  static bool _hasRepeatedDigits(String value) =>
      value.length > 1 && value.split('').toSet().length == 1;
