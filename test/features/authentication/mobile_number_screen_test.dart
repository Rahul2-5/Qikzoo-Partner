...
void main() {
  ...
  testWidgets('a valid signup number forwards its flow to OTP', (tester) async {
    ...
    // Signup mode also renders a "Full name" AppTextField above the phone
    // field (both wrap a plain TextField with no distinguishing key), so
    // find.byType(TextField) now matches two — the phone field is the
    // last one in the form. A name is now also required before Continue
    // succeeds in signup mode (MobileNumberScreen._onContinue rejects an
    // invalid name with its own snackbar), so it must be filled in too.
    await tester.enterText(find.byType(TextField).first, 'Test Rider');
