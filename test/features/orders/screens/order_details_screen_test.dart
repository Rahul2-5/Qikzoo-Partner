...
void main() {
  ...
  testWidgets('shows restaurant, customer, and earnings for a delivered order',
    ...
    expect(find.text('#BR-1'), findsOneWidget);
    expect(find.text('Spice Route Kitchen'), findsOneWidget);
    expect(find.text('221B Baker Street', skipOffstage: false), findsOneWidget);
  ...
  testWidgets('shows the full status timeline from the detail response', (tester) async {
    ...
    // The timeline card is the last item in the ListView, beyond both the
    // viewport and the sliver's build cache extent — skipOffstage: false
    // alone isn't enough here (that only bypasses Offstage-style hidden
    // widgets, not slivers that haven't been built yet), so this actually
    // scrolls the list until the card is built and visible.
    await tester.scrollUntilVisible(
      find.text('Order Timeline', skipOffstage: false),
      300,
