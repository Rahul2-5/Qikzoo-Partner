...
void main() {
  ...
  group('DioRiderOrdersRepository order-action endpoints', () {
    ...
    test('scanPickupQr posts the scanned token to the scan-pickup-qr endpoint', () async {
      final adapter = FakeHttpClientAdapter((options) {
        expect(options.path, ApiEndpoints.riderOrderScanPickupQr('rider-order-1'));
        expect(options.data, {'token': 'qr-token-1'});
        return jsonResponse('{"data":{}}', 201);
      });
      await buildRepository(adapter).scanPickupQr('rider-order-1', 'qr-token-1');
    });
