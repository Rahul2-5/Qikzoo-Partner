...
void main() {
  ...
  group('DioDispatchRepository.accept / reject', () {
    test('accept posts to the attempt-scoped accept endpoint and returns the new RiderOrder id',
        () async {
      final adapter = FakeHttpClientAdapter((options) {
        expect(options.path, ApiEndpoints.riderDispatchAccept('attempt-1'));
        expect(options.method, 'POST');
        return jsonResponse('{"data":{"id":"rider-order-1"}}', 201);
      });
      final repo = buildRepository(adapter);

      final riderOrderId = await repo.accept('attempt-1');

      expect(adapter.requestedPaths, contains('/rider/dispatch/attempt-1/accept'));
      expect(riderOrderId, 'rider-order-1');
    });

    test('accept throws if the response has no order id to hydrate the active-order screen with',
        () async {
      final adapter = FakeHttpClientAdapter((options) => jsonResponse('{"data":{}}', 201));
      final repo = buildRepository(adapter);

      await expectLater(repo.accept('attempt-1'), throwsA(isA<StateError>()));
    });
